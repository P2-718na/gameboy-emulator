...
Graphics   &\PBS\centering 🟠   &PPU Timings are machine-\/cycle accurate, but there is no FIFO implementation. Each scanline gets drawn all at once.    \\\cline{1-3}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{./gameboy\ \ \ \#\ Run\ standalone\ emulator}
\DoxyCodeLine{}
...
\end{DoxyCode}
 The current organization of the include directories is a bit cumbersome and I am planning on improving it. If you want to run Blargg\textquotesingle{}s tests, make sure to also copy the test ROMs to the binary folder.
...
Documentation is automatically generated for the {\ttfamily Gameboy} library. The documentation for the latest release is available \href{https://p2-718na.github.io/gameboy-emulator/html/index.html}{\texttt{ here}}.\hypertarget{md_README_autotoc_md17}{}\doxysubsection{On code clarity}\label{md_README_autotoc_md17}
...
\item cpplint.\+py to check for some additional details that Clang-\/\+Tidy could not pick up (\href{https://google.github.io/styleguide/cppguide.html}{\texttt{ Google styleguide}}).
\end{DoxyEnumerate}

I also run the code through {\itshape Valgrind Memcheck} and I made sure that there are no memory-\/related errors in my code.
